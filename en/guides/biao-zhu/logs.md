...
The **Logs** feature is designed to observe and annotate the performance of Mlchain applications. Mlchain records logs for all interactions with the application, whether through the WebApp or API. If you are a Prompt Engineer or LLM operator, it will provide you with a visual experience of LLM application operations.
...
{% hint style="info" %}
These annotations will be used for model fine-tuning in future versions of Mlchain to improve model accuracy and response style. The current preview version only supports annotations.
{% endhint %}
