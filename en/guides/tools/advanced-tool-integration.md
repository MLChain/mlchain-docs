...
Before starting with this advanced guide, please make sure you have a basic understanding of the tool integration process in Mlchain. Check out [Quick Integration](https://docs.mlchain.khulnasoft.com/tutorials/quick-tool-integration) for a quick run through.
...
Mlchain supports various message types such as `text`, `link`, `image`, and `file BLOB`. You can return different types of messages to the LLM and users through the following interfaces.
...
This tool takes in an user\_id and the text to be summarized, and returns the summarized text. Mlchain will use the default model of the current workspace to summarize the long text.
...
This tool takes in web page link to be crawled and a user\_agent (which can be empty), and returns a string containing the information of the web page. The `user_agent` is an optional parameter that can be used to identify the tool. If not passed, Mlchain will use the default `user_agent`.
