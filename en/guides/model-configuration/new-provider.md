...
For instance, with OpenAI, we can fine-tune multiple models based on gpt-turbo-3.5, all under the same **api_key**. When configured as `fetch-from-remote`, developers only need to configure a unified **api_key** to allow Mlchain Runtime to fetch all the developer's fine-tuned models and connect to Mlchain.
...
* Create a provider YAML file and write it according to the [Provider Schema](https://github.com/mlchain/mlchain/blob/main/api/core/model_runtime/docs/en_US/schema.md).
...
* If there are predefined models, create same-named YAML files under the model `module`, such as `claude-2.1.yaml`, and write them according to the [AI Model Entity](https://github.com/mlchain/mlchain/blob/main/api/core/model_runtime/docs/en_US/schema.md#aimodelentity).
...
If the connected provider offers customizable models, such as `OpenAI` which provides fine-tuned models, we need to add [`model_credential_schema`](https://github.com/mlchain/mlchain/blob/main/api/core/model_runtime/docs/en_US/schema.md). Taking `OpenAI` as an example:
...
You can also refer to the [YAML configuration information](https://github.com/mlchain/mlchain/blob/main/api/core/model_runtime/docs/en_US/schema.md) in the directories of other providers under the `model_providers` directory.
...
[**Adding Predefined Models**](https://docs.mlchain.khulnasoft.com/v/zh-hans/guides/model-configuration/predefined-model)**👈🏻**

For predefined models, we can connect them by simply defining a YAML file and implementing the calling code.

[**Adding Custom Models**](https://docs.mlchain.khulnasoft.com/v/zh-hans/guides/model-configuration/customizable-model) **👈🏻**
