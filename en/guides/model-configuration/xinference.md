...
[Xorbits inference](https://github.com/xorbitsai/inference) is a powerful and versatile library designed to serve language, speech recognition, and multimodal models, and can even be used on laptops. It supports various models compatible with GGML, such as chatglm, baichuan, whisper, vicuna, orca, etc. And Mlchain supports connecting to Xinference deployed large language model inference and embedding capabilities locally.
...
2.  Start Xinference locally:
    ...
    > If you use the Mlchain Docker deployment method, you need to pay attention to the network configuration to ensure that the Mlchain container can access the endpoint of Xinference. The Mlchain container cannot access localhost inside, and you need to use the host IP address.
...
5.  After the model is deployed, connect the deployed model in Mlchain.
    ...
    * Model UID: `2c886330-8849-11ee-9518-43b0b8f40bea`
    ...
    Click "Save" to use the model in the mlchain application.

Mlchain also supports using [Xinference builtin models](https://github.com/xorbitsai/inference/blob/main/README.md#builtin-models) as Embedding models, just select the Embeddings type in the configuration box.
