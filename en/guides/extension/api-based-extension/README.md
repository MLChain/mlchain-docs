...
Before extending module capabilities, prepare an API and an API Key for authentication, which can also be automatically generated by Mlchain. In addition to developing the corresponding module capabilities, follow the specifications below so that Mlchain can invoke the API correctly.
...
Mlchain will invoke your API according to the following specifications:
...
When configuring API-based Extension in Mlchain, Mlchain will send a request to the API Endpoint to verify the availability of the API. When the API Endpoint receives `point=ping`, the API should return `result=pong`, as follows:
...
The default port is 8000. The complete address of the API is: `http://127.0.0.1:8000/api/mlchain/receive`with the configured API Key '123456'.

<pre><code><strong>uvicorn main:app --reload --host 0.0.0.0
</strong></code></pre>

#### Configure this API in Mlchain.
...
When debugging the App, Mlchain will request the configured API and send the following content (example):
...
Since Mlchain's cloud version can't access internal network API services, you can use Ngrok to expose your local API service endpoint to the public internet for cloud-based debugging of local code. The steps are:
...
* For example, to expose your locally running service, replace the example URL `http://127.0.0.1:8000/api/mlchain/receive` with `https://177e-159-223-41-52.ngrok-free.app/api/mlchain/receive`.

Now, this API endpoint is accessible publicly. You can configure this endpoint in Mlchain for local debugging. For the configuration steps, consult the appropriate documentation or guide.
