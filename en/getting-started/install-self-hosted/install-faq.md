...
The database, configured storage, and vector database data need to be backed up. If deployed in Docker Compose mode, all data content in the `mlchain/docker/volumes` directory can be directly backed up.

### 10. Why is Docker deploying Mlchain and starting OpenLLM locally using 127.0.0.1, but unable to access the local port?

`127.0.0.1` is the internal address of the container, and the server address configured by Mlchain requires the host LAN IP address.
