...
控制台 API 后端 URL，用于拼接授权回调，传空则为同域。范例：`https://api.console.mlchain.khulnasoft.com`。

#### CONSOLE\_WEB\_URL

控制台 web **前端** URL，用于拼接部分前端地址，以及 CORS 配置使用，传空则为同域。范例：`https://console.mlchain.khulnasoft.com`

#### SERVICE\_API\_URL

Service API URL，用于**给前端**展示 Service API Base URL，传空则为同域。范例：`https://api.mlchain.khulnasoft.com`

#### APP\_API\_URL

WebApp API 后端 URL，用于声明**前端** API 后端地址，传空则为同域。范例：`https://app.mlchain.khulnasoft.com`

#### APP\_WEB\_URL

WebApp URL，用于**给前端**展示 WebAPP API Base URL，传空则为同域。范例：`https://api.app.mlchain.khulnasoft.com`
...
是否开启检查版本策略，若设置为 false，则不调用 `https://updates.mlchain.khulnasoft.com` 进行版本检查。由于目前国内无法直接访问基于 CloudFlare Worker 的版本接口，设置该变量为空，可以屏蔽该接口调用。
...
*   CELERY\_BROKER\_URL
    ...
    范例：`redis://:mlchainai123456@redis:6379/1`
*   BROKER\_USE\_SSL
...
详细配置可参考：[跨域/身份相关指南](https://docs.mlchain.khulnasoft.com/v/zh-hans/learn-more/faq/install-faq#id-3.-an-zhuang-shi-hou-wu-fa-deng-lu-deng-lu-cheng-gong-dan-hou-xu-jie-kou-jun-ti-shi-401)
...
* AZURE\_BLOB\_ACCOUNT\_NAME: your-account-name 如 'mlchainai'
* AZURE\_BLOB\_ACCOUNT\_KEY: your-account-key 如 'mlchainai'
* AZURE\_BLOB\_CONTAINER\_NAME: your-container-name 如 'mlchainai-container'
...
* MAIL\_TYPE
  * resend
    * MAIL\_DEFAULT\_SEND\_FROM\
      发件人的电子邮件名称，例如：no-reply [no-reply@mlchain.khulnasoft.com](mailto:no-reply@mlchain.khulnasoft.com)，非必需。
    * RESEND\_API\_KEY\
...
详见可用的[工具](https://github.com/mlchain/mlchain/blob/main/api/core/tools/provider/_position.yaml) 和 [模型供应商](https://github.com/mlchain/mlchain/blob/main/api/core/model_runtime/model_providers/_position.yaml)。
...
控制台 URL，用于拼接授权回调、控制台前端地址，以及 CORS 配置使用，传空则为同域。范例：`https://console.mlchain.khulnasoft.com`。

#### API\_URL

> ⚠️ 修改于 0.3.8，于 0.4.9 废弃，替代为 `SERVICE_API_URL`。

API Url，用于**给前端**展示 Service API Base Url，传空则为同域。范例：`https://api.mlchain.khulnasoft.com`

#### APP\_URL

> ⚠️ 修改于 0.3.8，于 0.4.9 废弃，替代为 `APP_API_URL` 和 `APP_WEB_URL`。

WebApp Url，用于声明**前端** API 后端地址，传空则为同域。范例：`https://app.mlchain.khulnasoft.com`
