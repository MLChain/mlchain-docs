...
工具可以扩展 LLM 的能力，比如联网搜索、科学计算或绘制图片，赋予并增强了 LLM 连接外部世界的能力。Mlchain 提供了两种工具类型：**第一方工具**和**自定义工具**。

你可以直接使用 Mlchain 生态提供的第一方内置工具，或者轻松导入自定义的 API 工具（目前支持 OpenAPI / Swagger 和 OpenAI Plugin 规范）。

#### 工具的作用：

1. 工具使用户可以在 Mlchain 上创建更强大的 AI 应用，如你可以为智能助理型应用（Agent）编排合适的工具，它可以通过任务推理、步骤拆解、调用工具完成复杂任务。
...
{% hint style="info" %}
欢迎您为 Mlchain 贡献自己开发的工具，关于如何贡献的具体方法请查看 [Mlchain 开发贡献文档](https://github.com/mlchain/mlchain/blob/main/CONTRIBUTING.md)，您的任何支持对我们都是极为宝贵的。
{% endhint %}

#### 第一方工具授权

若你需要直接使用 Mlchain 生态提供的第一方内置工具，你需要在使用前配置相应的凭据。
...
您也可以使用 [mlchain-tools-worker](https://github.com/crazywoola/mlchain-tools-worker) 来快速部署自定义工具。该工具提供了：

* 可以导入 Mlchain 的路由 `https://mlchaintoolsworker.yourname.workers.dev/doc`, 提供了 OpenAPI 兼容的接口文档
