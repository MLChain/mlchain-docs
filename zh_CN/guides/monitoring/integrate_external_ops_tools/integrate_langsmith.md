...
本章节将指引你注册 LangSmith 并将其集成至 Mlchain 平台内。

#### 1. 注册/登录 [LangSmith](https://www.langchain.com/langsmith)

#### 2. 创建项目

在 LangSmith 内创建项目，登录后在主页点击 **New Project** 创建一个自己的项目，**项目**将用于与 Mlchain 内的**应用**关联进行数据监测。
...
#### 4. 将 LangSmith 集成至 Mlchain 平台

在 Mlchain 应用内配置 LangSmith。打开需要监测的应用，在左侧边菜单内打开**监测**，点击页面内的**配置。**
...
配置完成后， Mlchain 内应用的调试或生产数据可以在 LangSmith 查看监测数据。

<figure><img src="../../../.gitbook/assets/image (17).png" alt=""><figcaption><p>在 Mlchain 内调试应用</p></figcaption></figure>
