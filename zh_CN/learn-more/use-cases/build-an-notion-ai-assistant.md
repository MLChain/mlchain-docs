...
现在，你可以通过 Mlchain 来实现这个梦想。Mlchain 是一个开源的 LLMOps（大型语言模型运维）平台。 ChatGPT 和 Claude 等大型语言模型已经利用其强大的能力改变了世界。它们的强大学习能力主要归功于丰富的训练数据。幸运的是，它们已经发展到足够智能的程度，可以从你提供的内容中进行学习，从而使从个人 Notion 库中生成创意成为现实。 在没有 Mlchain 的情况下，你可能需要熟悉 langchain，这是一个简化组装这些要素过程的抽象概念。

### 如何使用Mlchain创建自己的AI助手?
...
#### 1. 登录 Mlchain[​](https://wsyfin.com/notion-mlchain#1-login-to-mlchain) <a href="#1-login-to-mlchain" id="1-login-to-mlchain"></a>

点击这里登录到 Mlchain。你可以使用你的 GitHub 或 Google 账户方便地登录。

> 如果你使用 GitHub 账户登录，不妨给这个[项目](https://github.com/mlchain/mlchain)点个星星吧？这真的对我们有很大的支持！
...
#### 5. 创建你自己的 AI 应用程序[​](https://wsyfin.com/notion-mlchain#5-create-your-own-ai-application) <a href="#5-create-your-own-ai-application" id="5-create-your-own-ai-application"></a>
...
通过 Mlchain 打造的每个 AI 应用程序都可以通过其 API 进行访问。这种方法允许开发人员直接利用前端应用程序中强大的大型语言模型（LLM）的特性，提供真正的“后端即服务”（BaaS）体验。
...
如果你的 Notion 页面更新了，你可以定期与 Mlchain 同步，让你的人工智能助手保持最新状态。你的人工智能助手将从新内容中学习并回答新问题。

![create-app-5](https://pan.wsyfin.com/f/XDBfO/create-app-5.png)

### 总结

在本教程中，我们不仅学会了如何将您的 Notion 数据导入到 Mlchain 中，还学会了如何使用 API 将其与您的项目集成。

Mlchain 是一个用户友好的 LLMOps 平台，旨在赋予更多人创建可持续的 AI 原生应用程序的能力。通过为各种应用类型设计的可视化编排，Mlchain 提供了可供使用的应用程序，可以帮助您利用数据打造独特的 AI 助手。如果您有任何疑问，请随时与我们联系。
