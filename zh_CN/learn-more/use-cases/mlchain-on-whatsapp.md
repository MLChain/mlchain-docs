# 使用Mlchain 和Twilio构建WhatsApp机器人
...
随着人工智能的崛起，聊天机器人变得更聪明，更个性化，更直观。在本文中，我们将向您展示如何使用使用Mlchain和Twilio将其与WhatsApp集成。

您将首先使用FastAPI 接入Mlchain设置后端，然后，您将集成Twilio的WhatsApp消息API，允许客户与您的WhatsApp聊天机器人开始对话。
...
## 3. 创建Mlchain基础编排聊天助手应用 （節錄自[手摸手教你把 Mlchain 接入微信生态](./mlchain-on-wechat.md))
...
MLCHAIN_URL=<在(3)獲取的Mlchain API服务器地址>
MLCHAIN_API_KEY=<在(3)獲取的Mlchain API密钥>
```
...
##### 5.5.2 将WhatsApp号码作为Mlchain会话ID，确保用户持续保持该会话
...
掃6.1 頁面的二維碼進入WhatsApp 沙盒環境，然後发送WhatsApp消息，并等待您的AI聊天机器人的回复。尝试向AI聊天机器人提问您可以向Mlchain 聊天助手提问的任何问题。
