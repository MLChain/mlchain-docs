...
> OpenAI TTSを使用する場合、システムにFFmpegをインストールする必要があります。詳細は[リンク](https://docs.mlchain.khulnasoft.com/v/ja-jp/learn-more/faq/install-faq#id-15-tekisutomigeniopenai-error-ffmpeg-is-not-installedtoiuergashitano)を参照してください。
...
4.  依存関係をインストール
    ...
    Mlchain APIサービスは依存関係を管理するために[Poetry](https://python-poetry.org/docs/)を使用します。環境を有効にするには、`poetry shell`を実行できます。
...
7.  Workerサービスを開始
    ...
    - ** ---------- .> results:     postgresql://postgres:**@localhost:5432/mlchain
...
最後に、http://127.0.0.1:3000 にアクセスすると、ローカルデプロイメントされたMlchainを使用できます。