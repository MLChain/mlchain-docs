# MlchainとTwilioを使用してWhatsAppボットを構築する方法
...
人工知能の台頭に伴い、チャットボットはより賢く、個別化され、直感的になってきました。本記事では、MlchainとTwilioを使用してWhatsAppと統合する方法を説明します。

まず、FastAPIを使用してMlchainのバックエンドを設定し、その後、TwilioのWhatsAppメッセージAPIを統合し、顧客がWhatsAppチャットボットと対話できるようにします。
...
## 3. Mlchainの基本的なチャットアシスタントアプリケーションの作成（[MlchainをWeChatエコシステムに接続する方法](./mlchain-on-wechat.md)からの抜粋）

まず、[Mlchain公式アプリケーションプラットフォーム](https://cloud.mlchain.khulnasoft.com/signin)にログインします。GithubまたはGoogleのアカウントでログインすることができます。また、Mlchain公式チュートリアル[Docker Composeによるデプロイ | 日本語 | Mlchain](https://docs.mlchain.khulnasoft.com/v/ja-jp/getting-started/install-self-hosted/docker-compose)を参照して、プライベートデプロイを行うことも可能です。Mlchainはオープンソースプロジェクトであり、プライベートデプロイをサポートしています。

<figure><img src="../../.gitbook/assets/mlchain-on-wechat/create-basic-chatbot.jpg" alt=""><figcaption></figcaption></figure>

ログイン後、Mlchainのページに移動し、以下のステップに従って基本的なチャットアシスタントアプリケーションを作成します。
...
キーを保存した後、右上のAPIサーバーも確認してください。Mlchain公式のアプリケーションの場合、APIサーバーのアドレスは「https://api.mlchain.khulnasoft.com/v1」です。プライベートデプロイの場合は、自分のAPIサーバーアドレスを確認してください。
...
<figure><img src="../../.gitbook/assets/mlchain-on-whatsapp/twilio1.png" alt=""><figcaption></figcaption></figure>
...
```
git clone https://github.com/somethingwentwell/mlchain-twilio-whatsapp
```
...
MLCHAIN_URL=<3で取得したMlchain APIサーバーアドレス>
MLCHAIN_API_KEY=<3で取得したMlchain APIキー>
```
...
```
mlchain-whatsapp-1  | INFO:     Started server process [68]
mlchain-whatsapp-1  | INFO:     Waiting for application startup.
mlchain-whatsapp-1  | INFO:     Application startup complete.
```
...
<figure><img src="../../.gitbook/assets/mlchain-on-whatsapp/lt1.png" alt=""><figcaption></figcaption></figure>
...
##### 5.5.2 WhatsApp番号をMlchainセッションIDとして使用し、ユーザーのセッションを継続的に保持
...
```python
    ...
    headers = {  
        ...
        'Authorization': f"Bearer {mlchain_api_key}",  
    }  
    data = {  
        'inputs': {},  
        'query': Body,  
...
<figure><img src="../../.gitbook/assets/mlchain-on-whatsapp/twilio2.png" alt=""><figcaption></figcaption></figure>
...
<figure><img src="../../.gitbook/assets/mlchain-on-whatsapp/twilio3.png" alt=""><figcaption></figcaption></figure>

## 7. WhatsAppのテスト

6.1のページでQRコードをスキャンしてWhatsAppサンドボックス環境に入り、WhatsAppメッセージを送信してAIチャットボットの応答を待ちます。Mlchainチャットアシスタントに質問できることをAIチャットボットに試してみてください。

<figure><img src="../../.gitbook/assets/mlchain-on-whatsapp/whatsapp1.jpg" style="width:300px;" alt=""><figcaption></figcaption></figure>
